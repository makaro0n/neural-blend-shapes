...
from meshcnn.models.layers.mesh_prepare import fill_mesh, extract_features
...
class Mesh:
    ...
    @staticmethod
    def copy_from_mesh(mesh, verts, hold_history=False, requires_feature=False):
        new_mesh = Mesh(hold_history=hold_history)
        new_mesh.vs = verts.detach().cpu().numpy() if verts is not None else None
        new_mesh.edges = mesh.edges.copy()
        new_mesh.gemm_edges = mesh.gemm_edges.copy()
        new_mesh.edges_count = mesh.edges_count
        new_mesh.ve = mesh.ve.copy()
        new_mesh.v_mask = mesh.v_mask.copy()
        new_mesh.edge_areas = mesh.edge_areas.copy()
        new_mesh.sides = mesh.sides.copy()
        if requires_feature:
            new_mesh.features = extract_features(new_mesh)
        return new_mesh
    ...
    def merge_vertices(self, edge_id):
        ...
        mask = self.edges == edge[1]    # todo: Improve performance
    ...
    def export(self, file=None, vcolor=None):
        if self.is_train:
            return
        if file is None:
            file = './results/ae_mesh/%s_%d.obj' % (self.export_folder, self.pool_count)
