...
def get_mesh_path(file: str, num_aug: int):
    ...
    load_file = os.path.join(
        load_dir, "%s_%03d.npz" % (prefix, np.random.randint(0, num_aug))
    )
...
def from_scratch(file, opt=None, requires_feature=False):
    class MeshPrep:
    ...
    mesh_data.ve = None  # neighbor edges of a vertex
    ...
    if requires_feature:
        mesh_data.features = extract_features(
            mesh_data
        )  # We don't need MeshCNN feature in our setting
    else:
        mesh_data.features = None
    return mesh_data
...
def slide_verts(mesh, prct):
    ...
    dihedral = dihedral_angle(
        mesh, edge_points
    ).squeeze()  # todo make fixed_division epsilon=0
    ...
    for edge_key in edges2flip:
        ...
        if dihedral[edge_key] > 2.7:
            ...
            if edge_info[3] == -1:
                continue
            new_edge = tuple(
                sorted(list(set(faces[edge_info[2]]) ^ set(faces[edge_info[3]])))
            )
