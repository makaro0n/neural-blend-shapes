        ...
        if bpy.app.background:
            ...
            for vg in range(weight.shape[1]):
                ...
                for i in range(weight[:, vg].shape[0]):
                    if weight[:, vg][i] > 0:
                        mesh.vertex_groups[vg_index].add(
                            [i], weight[:, vg][i], "REPLACE"
                        )
                    ...
                    for n in range(frame_num):
                        for i in range(coff.shape[1]):
                            if coff[n][i] >= 0:
                                ...
                                mesh.data.shape_keys.key_blocks[key_name].value = coff[
                                    n
                                ][i]
                                mesh.data.shape_keys.key_blocks[
                                    key_name
                                ].keyframe_insert(data_path="value", frame=n + 1)
                                ...
                                key_name = str(i).zfill(3) + "_neg"
                                mesh.data.shape_keys.key_blocks[key_name].value = (
                                    -1 * coff[n][i]
                                )
                                mesh.data.shape_keys.key_blocks[
                                    key_name
